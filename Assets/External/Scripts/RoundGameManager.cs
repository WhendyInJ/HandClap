...
public enum RoundGameState
{
    ...
    /// <summary>라운드 1 전 스승 코치 — 단계별 텍스트·시간. 전투 입력은 켤 수 있음.</summary>
    TutorialCoach,
...
/// <summary><see cref="RoundGameManager"/> 라운드 1 전 코치 튜토리얼 한 단계.</summary>
[Serializable]
...
public class RoundGameManager : MonoBehaviour
{
    ...
    /// <summary>(단계 인덱스 0-based, 전체 단계 수, 이번 단계 남은 초)</summary>
    ...
    void HandlePlayerBasicAttackStarted()
    {
        ...
        // 반응형/경합 튜토리얼 단계에서는 버튼 입력이 아니라 실제 결과로 카운트한다.
    ...
    void TickCoachTutorialStep(RoundTutorialCoachStep step)
    {
        ...
        {
            bool started = step.forceSimultaneousAttackClash
                ? false
                : TryTriggerTutorialEnemyAction(step);
    ...
    bool TryTriggerTutorialEnemyAction(RoundTutorialCoachStep step)
    {
        ...
        return enemyController != null && enemyController.TryBasicAttackOnly();
    }
    ...
    bool TryTriggerTutorialForcedClash()
    {
        ...
        bool enemyStarted = enemyAiController != null
            ...
            : enemyController.TryBasicAttackOnly();

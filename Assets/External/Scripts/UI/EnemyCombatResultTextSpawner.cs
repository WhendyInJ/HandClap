...
public class EnemyCombatResultTextSpawner : MonoBehaviour
{
    ...
    {
        if (!TryGetEnemyResultMessage(eventData, out string message))
            return;

        SpawnMessage(message);
    }

    bool TryGetEnemyResultMessage(CombatEventData eventData, out string message)
    {
        message = null;
        ...
        switch (eventData.Kind)
        {
            case CombatEventKind.FeintFailed:
                if (eventData.Actor == enemyController)
